  ...
  const masteryEvidence = getDatabase()
    .prepare(
      [
        ...
        "WHERE mastery.student_model_version_id = ? AND mastery.skill_key = ? AND item.in_taxonomy_scope = 1",
  ...
  const candidates = database
    .prepare(
      [
        ...
        "JOIN assignment_items AS item ON item.id = answer.assignment_item_id",
        "JOIN problems AS problem ON problem.id = item.problem_id",
        "WHERE submission.class_id = ? AND diagnosis.outcome = 'MISCONCEPTION' AND item.in_taxonomy_scope = 1",
        "AND diagnosis.id = (",
  ...
  const targets = database
    .prepare(
      [
        ...
        "WHERE item.class_id = ? AND item.in_taxonomy_scope = 1 AND assignment.status = 'READY' AND assignment.archived_at IS NULL",
