  ...
  const row = getDatabase()
    .prepare(
      [
        ...
        "AND diagnosis.outcome = 'MISCONCEPTION' AND diagnosis.misconception_id = ? AND item.in_taxonomy_scope = 1",
        "AND diagnosis.id = (",
  ...
  const row = getDatabase()
    .prepare(
      [
        ...
        "AND diagnosis.outcome = 'MISCONCEPTION' AND diagnosis.taxonomy_version = ? AND item.in_taxonomy_scope = 1",
    ...
    const supportingDiagnoses = database
      .prepare(
        [
          ...
          "JOIN diagnoses AS diagnosis ON diagnosis.answer_version_id = answer_version.id",
          "WHERE submission.class_id = ? AND submission.membership_id = ?",
          "AND diagnosis.outcome = 'MISCONCEPTION' AND item.in_taxonomy_scope = 1",
    ...
    const correctDiagnoses = database
      .prepare(
        [
          ...
          "JOIN problems AS problem ON problem.id = item.problem_id",
          "WHERE submission.class_id = ? AND submission.membership_id = ?",
          "AND problem.domain = ? AND diagnosis.outcome = 'CORRECT' AND item.in_taxonomy_scope = 1 AND COALESCE(diagnosis.correction_verdict, 'CORRECT') = 'CORRECT'",
          "AND julianday(diagnosis.created_at) <= julianday(?)",
          "AND diagnosis.id = (",
          "SELECT latest.id FROM diagnoses AS latest",
          "JOIN answer_versions AS latest_version ON latest_version.id = latest.answer_version_id",
